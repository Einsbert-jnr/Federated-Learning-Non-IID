...
# Load dataset
ddos = pd.read_csv("I:\\My Drive\\Colab Notebooks\\datasets\\data\\Prepared\\ddos_trainval_set.csv")
print(ddos.head())

ddos = shuffle(ddos).reset_index(drop=True)
# Split dataset
X = ddos.drop(columns=['Label'])
y = ddos['Label']
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.X_train, self.y_train, validation_data=(self.X_val, self.y_val), epochs=5, batch_size=128, verbose=VERBOSE)
