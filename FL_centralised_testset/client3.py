...
# Load dataset
bruteforce = pd.read_csv("I:\\My Drive\\Colab Notebooks\\datasets\\data\\Prepared\\bruteforce_trainval_set.csv")
print(bruteforce.head())

bruteforce = shuffle(bruteforce).reset_index(drop=True)
# Split dataset
X = bruteforce.drop(columns=['Label'])
y = bruteforce['Label']
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.X_train, self.y_train, validation_data=(self.X_val, self.y_val), epochs=20, batch_size=128, verbose=VERBOSE)
